// Decompiled by Jad v1.5.8e. Copyright 2001 Pavel Kouznetsov.
// Jad home page: http://www.geocities.com/kpdus/jad.html
// Decompiler options: braces fieldsfirst space lnc 


public final class agx extends kwm
{

    public Integer A;
    public Integer B;
    public Integer C;
    public Integer D;
    public Integer E;
    public Integer F;
    public agy G;
    public jzj H;
    public String I;
    public Boolean J;
    public Integer K;
    public Integer L;
    public Integer M;
    public Integer a;
    public Integer b;
    public Integer c;
    public Integer d;
    public Integer e;
    public Integer f;
    public Integer g;
    public Integer h;
    public Integer i;
    public Integer j;
    public Integer k;
    public Integer l;
    public Integer m;
    public Integer n;
    public Integer o;
    public Integer p;
    public Integer q;
    public agv r;
    public Integer s;
    public Integer t;
    public agw u;
    public agw v;
    public agw w;
    public agw x;
    public Integer y;
    public String z;

    public agx()
    {
        a = null;
        b = null;
        c = null;
        d = null;
        e = null;
        f = null;
        g = null;
        h = null;
        i = null;
        j = null;
        k = null;
        l = null;
        m = null;
        n = null;
        o = null;
        p = null;
        q = null;
        r = null;
        s = null;
        t = null;
        u = null;
        v = null;
        w = null;
        x = null;
        y = null;
        z = null;
        A = null;
        B = null;
        C = null;
        D = null;
        E = null;
        F = null;
        G = null;
        H = null;
        I = null;
        J = null;
        K = null;
        L = null;
        M = null;
        unknownFieldData = null;
        cachedSize = -1;
    }

    protected int computeSerializedSize()
    {
        int j1 = super.computeSerializedSize();
        int i1 = j1;
        if (a != null)
        {
            i1 = j1 + kwk.e(1, a.intValue());
        }
        j1 = i1;
        if (b != null)
        {
            j1 = i1 + kwk.e(2, b.intValue());
        }
        i1 = j1;
        if (c != null)
        {
            i1 = j1 + kwk.e(3, c.intValue());
        }
        j1 = i1;
        if (d != null)
        {
            j1 = i1 + kwk.e(4, d.intValue());
        }
        i1 = j1;
        if (e != null)
        {
            i1 = j1 + kwk.e(5, e.intValue());
        }
        j1 = i1;
        if (f != null)
        {
            j1 = i1 + kwk.e(6, f.intValue());
        }
        i1 = j1;
        if (g != null)
        {
            i1 = j1 + kwk.e(7, g.intValue());
        }
        j1 = i1;
        if (h != null)
        {
            j1 = i1 + kwk.e(8, h.intValue());
        }
        i1 = j1;
        if (i != null)
        {
            i1 = j1 + kwk.e(9, i.intValue());
        }
        j1 = i1;
        if (j != null)
        {
            j1 = i1 + kwk.e(10, j.intValue());
        }
        i1 = j1;
        if (k != null)
        {
            i1 = j1 + kwk.e(11, k.intValue());
        }
        j1 = i1;
        if (l != null)
        {
            j1 = i1 + kwk.e(12, l.intValue());
        }
        i1 = j1;
        if (m != null)
        {
            i1 = j1 + kwk.e(13, m.intValue());
        }
        j1 = i1;
        if (n != null)
        {
            j1 = i1 + kwk.e(14, n.intValue());
        }
        i1 = j1;
        if (o != null)
        {
            i1 = j1 + kwk.e(15, o.intValue());
        }
        j1 = i1;
        if (p != null)
        {
            j1 = i1 + kwk.e(16, p.intValue());
        }
        i1 = j1;
        if (q != null)
        {
            i1 = j1 + kwk.e(17, q.intValue());
        }
        j1 = i1;
        if (r != null)
        {
            j1 = i1 + kwk.d(18, r);
        }
        i1 = j1;
        if (s != null)
        {
            i1 = j1 + kwk.e(19, s.intValue());
        }
        j1 = i1;
        if (t != null)
        {
            j1 = i1 + kwk.e(20, t.intValue());
        }
        i1 = j1;
        if (u != null)
        {
            i1 = j1 + kwk.d(21, u);
        }
        j1 = i1;
        if (v != null)
        {
            j1 = i1 + kwk.d(22, v);
        }
        i1 = j1;
        if (w != null)
        {
            i1 = j1 + kwk.d(23, w);
        }
        j1 = i1;
        if (x != null)
        {
            j1 = i1 + kwk.d(24, x);
        }
        i1 = j1;
        if (y != null)
        {
            i1 = j1 + kwk.e(25, y.intValue());
        }
        j1 = i1;
        if (z != null)
        {
            j1 = i1 + kwk.b(26, z);
        }
        i1 = j1;
        if (A != null)
        {
            i1 = j1 + kwk.e(27, A.intValue());
        }
        j1 = i1;
        if (B != null)
        {
            j1 = i1 + kwk.e(28, B.intValue());
        }
        i1 = j1;
        if (C != null)
        {
            i1 = j1 + kwk.e(29, C.intValue());
        }
        j1 = i1;
        if (D != null)
        {
            j1 = i1 + kwk.e(30, D.intValue());
        }
        i1 = j1;
        if (E != null)
        {
            i1 = j1 + kwk.e(31, E.intValue());
        }
        j1 = i1;
        if (F != null)
        {
            j1 = i1 + kwk.e(32, F.intValue());
        }
        i1 = j1;
        if (G != null)
        {
            i1 = j1 + kwk.d(33, G);
        }
        j1 = i1;
        if (H != null)
        {
            j1 = i1 + kwk.d(34, H);
        }
        i1 = j1;
        if (I != null)
        {
            i1 = j1 + kwk.b(35, I);
        }
        j1 = i1;
        if (J != null)
        {
            J.booleanValue();
            j1 = i1 + (kwk.f(36) + 1);
        }
        i1 = j1;
        if (K != null)
        {
            i1 = j1 + kwk.e(37, K.intValue());
        }
        j1 = i1;
        if (L != null)
        {
            j1 = i1 + kwk.e(38, L.intValue());
        }
        i1 = j1;
        if (M != null)
        {
            i1 = j1 + kwk.e(39, M.intValue());
        }
        return i1;
    }

    public kws mergeFrom(kwj kwj1)
    {
_L42:
        int i1 = kwj1.a();
        i1;
        JVM INSTR lookupswitch 40: default 336
    //                   0: 345
    //                   8: 347
    //                   16: 361
    //                   24: 375
    //                   32: 389
    //                   40: 403
    //                   48: 417
    //                   56: 431
    //                   64: 445
    //                   72: 459
    //                   80: 473
    //                   88: 487
    //                   96: 501
    //                   104: 515
    //                   112: 529
    //                   120: 543
    //                   128: 557
    //                   136: 571
    //                   146: 585
    //                   152: 614
    //                   160: 628
    //                   170: 642
    //                   178: 671
    //                   186: 700
    //                   194: 729
    //                   200: 758
    //                   210: 772
    //                   216: 783
    //                   224: 797
    //                   232: 811
    //                   240: 825
    //                   248: 839
    //                   256: 914
    //                   266: 928
    //                   274: 957
    //                   282: 986
    //                   288: 997
    //                   296: 1011
    //                   304: 1058
    //                   312: 1110;
           goto _L1 _L2 _L3 _L4 _L5 _L6 _L7 _L8 _L9 _L10 _L11 _L12 _L13 _L14 _L15 _L16 _L17 _L18 _L19 _L20 _L21 _L22 _L23 _L24 _L25 _L26 _L27 _L28 _L29 _L30 _L31 _L32 _L33 _L34 _L35 _L36 _L37 _L38 _L39 _L40 _L41
_L1:
        if (super.storeUnknownField(kwj1, i1)) goto _L42; else goto _L2
_L2:
        return this;
_L3:
        a = Integer.valueOf(kwj1.f());
          goto _L42
_L4:
        b = Integer.valueOf(kwj1.f());
          goto _L42
_L5:
        c = Integer.valueOf(kwj1.f());
          goto _L42
_L6:
        d = Integer.valueOf(kwj1.f());
          goto _L42
_L7:
        e = Integer.valueOf(kwj1.f());
          goto _L42
_L8:
        f = Integer.valueOf(kwj1.f());
          goto _L42
_L9:
        g = Integer.valueOf(kwj1.f());
          goto _L42
_L10:
        h = Integer.valueOf(kwj1.f());
          goto _L42
_L11:
        i = Integer.valueOf(kwj1.f());
          goto _L42
_L12:
        j = Integer.valueOf(kwj1.f());
          goto _L42
_L13:
        k = Integer.valueOf(kwj1.f());
          goto _L42
_L14:
        l = Integer.valueOf(kwj1.f());
          goto _L42
_L15:
        m = Integer.valueOf(kwj1.f());
          goto _L42
_L16:
        n = Integer.valueOf(kwj1.f());
          goto _L42
_L17:
        o = Integer.valueOf(kwj1.f());
          goto _L42
_L18:
        p = Integer.valueOf(kwj1.f());
          goto _L42
_L19:
        q = Integer.valueOf(kwj1.f());
          goto _L42
_L20:
        if (r == null)
        {
            r = new agv();
        }
        kwj1.a(r);
          goto _L42
_L21:
        s = Integer.valueOf(kwj1.f());
          goto _L42
_L22:
        t = Integer.valueOf(kwj1.f());
          goto _L42
_L23:
        if (u == null)
        {
            u = new agw();
        }
        kwj1.a(u);
          goto _L42
_L24:
        if (v == null)
        {
            v = new agw();
        }
        kwj1.a(v);
          goto _L42
_L25:
        if (w == null)
        {
            w = new agw();
        }
        kwj1.a(w);
          goto _L42
_L26:
        if (x == null)
        {
            x = new agw();
        }
        kwj1.a(x);
          goto _L42
_L27:
        y = Integer.valueOf(kwj1.f());
          goto _L42
_L28:
        z = kwj1.j();
          goto _L42
_L29:
        A = Integer.valueOf(kwj1.f());
          goto _L42
_L30:
        B = Integer.valueOf(kwj1.f());
          goto _L42
_L31:
        C = Integer.valueOf(kwj1.f());
          goto _L42
_L32:
        D = Integer.valueOf(kwj1.f());
          goto _L42
_L33:
        int j1 = kwj1.f();
        switch (j1)
        {
        case 0: // '\0'
        case 1: // '\001'
        case 2: // '\002'
        case 3: // '\003'
        case 4: // '\004'
        case 5: // '\005'
        case 6: // '\006'
        case 7: // '\007'
        case 8: // '\b'
        case 9: // '\t'
            E = Integer.valueOf(j1);
            break;
        }
        continue; /* Loop/switch isn't completed */
_L34:
        F = Integer.valueOf(kwj1.f());
        continue; /* Loop/switch isn't completed */
_L35:
        if (G == null)
        {
            G = new agy();
        }
        kwj1.a(G);
        continue; /* Loop/switch isn't completed */
_L36:
        if (H == null)
        {
            H = new jzj();
        }
        kwj1.a(H);
        continue; /* Loop/switch isn't completed */
_L37:
        I = kwj1.j();
        continue; /* Loop/switch isn't completed */
_L38:
        J = Boolean.valueOf(kwj1.i());
        continue; /* Loop/switch isn't completed */
_L39:
        int k1 = kwj1.f();
        switch (k1)
        {
        case -1: 
        case 0: // '\0'
        case 1: // '\001'
            K = Integer.valueOf(k1);
            break;
        }
        continue; /* Loop/switch isn't completed */
_L40:
        int l1 = kwj1.f();
        switch (l1)
        {
        case -1: 
        case 0: // '\0'
        case 1: // '\001'
        case 2: // '\002'
            L = Integer.valueOf(l1);
            break;
        }
        continue; /* Loop/switch isn't completed */
_L41:
        M = Integer.valueOf(kwj1.f());
        if (true) goto _L42; else goto _L43
_L43:
    }

    public void writeTo(kwk kwk1)
    {
        if (a != null)
        {
            kwk1.a(1, a.intValue());
        }
        if (b != null)
        {
            kwk1.a(2, b.intValue());
        }
        if (c != null)
        {
            kwk1.a(3, c.intValue());
        }
        if (d != null)
        {
            kwk1.a(4, d.intValue());
        }
        if (e != null)
        {
            kwk1.a(5, e.intValue());
        }
        if (f != null)
        {
            kwk1.a(6, f.intValue());
        }
        if (g != null)
        {
            kwk1.a(7, g.intValue());
        }
        if (h != null)
        {
            kwk1.a(8, h.intValue());
        }
        if (i != null)
        {
            kwk1.a(9, i.intValue());
        }
        if (j != null)
        {
            kwk1.a(10, j.intValue());
        }
        if (k != null)
        {
            kwk1.a(11, k.intValue());
        }
        if (l != null)
        {
            kwk1.a(12, l.intValue());
        }
        if (m != null)
        {
            kwk1.a(13, m.intValue());
        }
        if (n != null)
        {
            kwk1.a(14, n.intValue());
        }
        if (o != null)
        {
            kwk1.a(15, o.intValue());
        }
        if (p != null)
        {
            kwk1.a(16, p.intValue());
        }
        if (q != null)
        {
            kwk1.a(17, q.intValue());
        }
        if (r != null)
        {
            kwk1.b(18, r);
        }
        if (s != null)
        {
            kwk1.a(19, s.intValue());
        }
        if (t != null)
        {
            kwk1.a(20, t.intValue());
        }
        if (u != null)
        {
            kwk1.b(21, u);
        }
        if (v != null)
        {
            kwk1.b(22, v);
        }
        if (w != null)
        {
            kwk1.b(23, w);
        }
        if (x != null)
        {
            kwk1.b(24, x);
        }
        if (y != null)
        {
            kwk1.a(25, y.intValue());
        }
        if (z != null)
        {
            kwk1.a(26, z);
        }
        if (A != null)
        {
            kwk1.a(27, A.intValue());
        }
        if (B != null)
        {
            kwk1.a(28, B.intValue());
        }
        if (C != null)
        {
            kwk1.a(29, C.intValue());
        }
        if (D != null)
        {
            kwk1.a(30, D.intValue());
        }
        if (E != null)
        {
            kwk1.a(31, E.intValue());
        }
        if (F != null)
        {
            kwk1.a(32, F.intValue());
        }
        if (G != null)
        {
            kwk1.b(33, G);
        }
        if (H != null)
        {
            kwk1.b(34, H);
        }
        if (I != null)
        {
            kwk1.a(35, I);
        }
        if (J != null)
        {
            kwk1.a(36, J.booleanValue());
        }
        if (K != null)
        {
            kwk1.a(37, K.intValue());
        }
        if (L != null)
        {
            kwk1.a(38, L.intValue());
        }
        if (M != null)
        {
            kwk1.a(39, M.intValue());
        }
        super.writeTo(kwk1);
    }
}
