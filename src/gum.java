...
public final class gum
{
    ...
    static float a(gun gun1, Property property, double d1)
    {
        float f;
        if (!gun1.a(property))
        {
            throw new guo("Cannot animate position if stage size was not defined");
        }
        f = 1.0F;
        if (property != gus.e) goto _L2; else goto _L1
_L1:
        f = gun1.a;
_L4:
        return f * (float)d1;
...
_L1:
    ...
    private Collection a(gun gun1)
    {
        JSONArray jsonarray = gun1.c.getJSONArray("animations");
        ArrayList arraylist = new ArrayList();
        int j = jsonarray.length();
        for (int i = 0; i < j; i++)
        {
            a(gun1, jsonarray.getJSONObject(i), ((Collection) (arraylist)));
        }

        return arraylist;
    }

    private void a(gun gun1, JSONObject jsonobject, Collection collection)
    {
        Object obj = gun1.d.get(jsonobject.getString("id"));
...
_L5:
        ...
        akeyframe[k] = Keyframe.ofFloat(f, a(gun1, ((Property) (akeyframe1)), d1));
        if (jsonobject != null)
        {
        ...
        jsonobject = new guq((float)jsonobject.getDouble("x1"), (float)jsonobject.getDouble("y1"), (float)jsonobject.getDouble("x2"), (float)jsonobject.getDouble("y2"));
_L6:
        k++;
...
_L10:
    ...
    {
        context = new gun(this, context, i, map, -1F, -1F);
        map = new AnimatorSet();
        map.playTogether(a(context));
        map.addListener(new gup(this, context));
        return map;
        context;
_L2:
        throw new guo(context);
        context;
        continue; /* Loop/switch isn't completed */
        context;
        if (true) goto _L2; else goto _L1
