...
final class avd
{
    ...
    public avd(Context context, ListView listview)
    {
        a = context;
        b = listview;
    }
    ...
    public void a(ContextMenu contextmenu, View view, android.view.ContextMenu.ContextMenuInfo contextmenuinfo, MenuInflater menuinflater, dqd dqd)
    {
label0:
        {
            if (view == b)
...
_L2:
        i = 1;
          goto _L4
_L3:
        menuinflater.inflate(g.hy, contextmenu);
        if (dqd != null)
        {
            contextmenu.add(0, 1000, 0, l.fZ);
        }
        if (!flag)
        {
            if (!flag1 || i > 0)
            {
                contextmenu.findItem(h.bJ).setVisible(false);
            }
            contextmenu.findItem(h.aO).setVisible(false);
        }
        if (contextmenuinfo instanceof android.widget.AdapterView.AdapterContextMenuInfo)
        {
            view = (MessageListItemWrapperView)((android.widget.AdapterView.AdapterContextMenuInfo)contextmenuinfo).targetView;
            if (view != null && (view.c() instanceof MessageListItemView))
            {
                ...
                {
                    contextmenu.findItem(h.aE).setVisible(false);
                }
                if (view.v())
                {
                ...
                if (!view.n())
                {
                    contextmenu.findItem(h.fC).setVisible(false);
                }
                view = view.e();
                if (!TextUtils.isEmpty(view))
                {
                    ...
                    do
                    {
                        ...
                        {
                            view = g.r(view);
                            contextmenu.add(0, 27, 0, a.getString(l.fA, new Object[] {
                                contextmenuinfo
        ...
        contextmenu.setHeaderTitle(l.fF);
        return;
        i = 0;
        flag1 = false;
        flag = false;
