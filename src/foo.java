...
public final class foo
{
    ...
    public static final void a(fqj fqj1)
    {
        fqj1.b("(mimetype IN ('vnd.android.cursor.item/email_v2','vnd.android.cursor.item/phone_v2'))");
    }

    public static final void a(fqj fqj1, boolean flag, Context context)
    {
        if (!flag)
        {
            if (android.os.Build.VERSION.SDK_INT >= 11)
            {
                if (a(context))
                {
                    fqj1.b("(contact_id IN (SELECT _id FROM default_directory))");
                }
            } else
            {
                fqj1.b("(in_visible_group=1)");
            }
        }
        context = a();
